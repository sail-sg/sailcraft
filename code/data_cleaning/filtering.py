...
class Filtering:
    ...
    def compute_flagged_words_ratio(
        ...
        nospaced_document = document.replace(' ','').lower()
        for flag_word in flagged_words:
            if flag_word in nospaced_document:
                flagged_words_ratio += 1.0/len(words)
    ...
    def filtering(
        ...
        if cond_check_number_words:
            if not Filtering.check_number_words(
                ...
                return False, 'cond_check_number_words'
        if cond_check_character_repetition_removal:
            if not Filtering.check_character_repetition_removal(
                ...
                return False, 'cond_check_character_repetition_removal'
        if cond_check_word_repetition_removal:
            if not Filtering.check_word_repetition_removal(
                ...
                return False, 'cond_check_word_repetition_removal'
        if cond_check_special_characters:
            if not Filtering.check_special_characters(
                ...
                return False, 'cond_check_special_characters'
        if cond_check_stopwords:
            if not Filtering.check_stopwords(
                ...
                return False, 'cond_check_stopwords'
        if cond_check_flagged_words:
            if not Filtering.check_flagged_words(
                ...
                return False, 'cond_check_flagged_words'
        if cond_check_lang_id:
            if not Filtering.check_lang_id(
                ...
                return False, 'cond_check_lang_id'
        if cond_check_perplexity:
            if not Filtering.check_perplexity(
                ...
                return False, 'cond_check_perplexity'
        return True, 'passed_all_check'
...
class FunctionDatasetFiltering:
    ...
    def __call__(self, example):
        keep_example, info_filtering = Filtering.filtering(
            document=example["text"],
        ...
        return keep_example, info_filtering
...
class DatasetFiltering:
    def __init__(
        ...
        self.dataset_shard_id=dataset_shard_id
        self.lang_dataset_id = lang_dataset_id
        self.path_fasttext_model = path_fasttext_model
        self.path_sentencepiece_model = path_sentencepiece_model
        self.path_kenlm_model = path_kenlm_model
        ...
        # self.path_dir_save_dataset = os.path.abspath(path_dir_save_dataset)
        

        self.reasons_counter = {
                "cond_check_number_words": 0, 
                "cond_check_character_repetition_removal": 0, 
                "cond_check_word_repetition_removal": 0, 
                "cond_check_special_characters": 0, 
                "cond_check_flagged_words": 0, 
                "cond_check_stopwords": 0,
                "cond_check_lang_id": 0, 
                "cond_check_perplexity": 0, 
            }  
        self.example_cases = {reason: [] for reason in self.reasons_counter}
    ...
    def filtering(self):
        ...
        def apply_filtering_and_record_reasons(dataset):
            def map_function(example, index):
                keep, reason = func_dataset_filtering(example)
                return {"keep": keep, "filter_reason": reason if not keep else None, "text": example['text']}
            return dataset.map(map_function, with_indices=True, num_proc=self.num_proc)

        self.ds = apply_filtering_and_record_reasons(self.ds)

        reasons_counter = {reason: 0 for reason in self.reasons_counter}
        for example in self.ds:
            reason = example['filter_reason']
            if reason!=None and len(self.example_cases[reason]) < 2:
                self.example_cases[reason].append(example['text'])
            if reason:
                reasons_counter[reason] += 1
        self.reasons_counter.update(reasons_counter)

        self.ds = self.ds.filter(lambda example: example['keep'], num_proc=self.num_proc)

    def save_dataset(self):
        pathlib.Path(self.path_dir_save_dataset).mkdir(parents=True, exist_ok=True)
        #save data as .arrow format
        path_dir_save_dataset = pathlib.PurePath(
            self.path_dir_save_dataset, self.dataset_alias
        )
        pathlib.Path(path_dir_save_dataset).mkdir(parents=True, exist_ok=True)
        path_dir_save_dataset = str(path_dir_save_dataset)
        self.ds.save_to_disk(path_dir_save_dataset)

    def report_filter_stat(self):
        table = PrettyTable()
        table.field_names = [Fore.BLUE + "Filter Condition" + Style.RESET_ALL, Fore.BLUE + "Count" + Style.RESET_ALL]
        table.align["Filter Condition"] = "l"
        table.align["Count"] = "l"

        for reason, count in self.reasons_counter.items():
            # print(f"{reason}: {count} examples filtered out") 
            table.add_row([reason, count])  

        print(table)

        case_data = []
        for reason, examples in self.example_cases.items():
            for example in examples:
                case_data.append({'Reason': reason, 'Example': example})
        df = pd.DataFrame(case_data)

        # log_folder_path = 'filtering_logs'
        log_folder_path = self.log_folder_path
        log_file_path = os.path.join(log_folder_path, "{}_filter_cases.xlsx".format(self.dataset_alias))
